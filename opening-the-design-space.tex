...
\begin{figure}
    ...
    \caption{Inference time for differently sized AI models on different Raspberry Pis and an Apple MacBook Air (M1, 16GB). All Pis can run AI predictions in $<5\textrm{ms}$.}
...
While quantitative testing is not a main focus of this paper, we replicated MDRNN inference speed testing previously reported in \cite{Martin2019} to compare saved model formats at Raspberry Pi versions. Model formats compared were the Keras library's native format (keras) and the optimised Tensorflow Lite format (tflite). The tflite format was faster on all platforms and our tests found that even the cheapest Raspberry Pi Zero 2 W can run AI predictions in $<5\textrm{ms}$, below the previous benchmark of 10ms~\cite{Martin2019}. For smaller models, the Raspberry Pi 5 can run AI predictions in less than 0.5ms.
Our tests revealed that while the Pi Zero 2 W had useful performance on smaller neural networks (considering number of LSTM units as a measure of size), it ran out of memory and did not complete tests on the larger 512-unit model.
...
This section describes five new intelligent musical instruments and the experience of using them in performance over a period of approximately two years (2024-2026).
In this autobiographical artistic research, these instruments were developed for the first author's use and tested in live performances, recordings, and demonstrations to expand knowledge about the design space for intelligent musical instruments. The performance style was free improvised music.
...
\begin{figure}
	...
	% \caption{An intelligent setup of the Roland S-1 and Keith McMillan QuNeo interfaces with a Raspberry Pi 4 as used in a duo performance in November 2025}
	\caption{An intelligent setup of the Roland S-1 and Keith McMillan QuNeo interfaces with a Raspberry Pi 5 as used in festival performances in January 2026}
\end{figure}
...
In this kind of \emph{intelligent setup}, the AI model mediates events in betweeen the different interfaces. 
The intelligent DAW idea explained above could enable this kind of extension by programming MIDI routings within the DAW itself, but a better solution for a DAW-less setup was to extend the MIDI mapping capability of our AI system to enable multiple input and output routings.
...
The new mapping enabled the 8 knobs to control the AI model with human control events mapped directly through the S-1. Similarly, control on the S-1 was still available. MIDI outputs to control change could be restricted to a specific output range enabling control over the LED rings.
This first setup worked well in performance, but the new flexibility of the MIDI mapping encouraged experimentation with interfaces beyond knobs. Substituting the X-Touch for a Keith McMillan QuNeo pad controller enabled control of and feedback from the 8 AI-linked parameters through touch sliders and additional note control with the drum pads. 
...
Its worth noting that at this point, I was more likely to perform with a Raspberry Pi 4 or 5 rather than the Zero 2 W used in early prototypes. Both the boot and inference times were faster on the full-size Raspberry Pi's, a practical and musical consideration for performance.
...
This work has described a two-year practice of intelligent instrument building and exploration in performances and recordings. The goal of this process was to expand the design space of intelligent musical instrument design by trying many prototypes. 

\subsection{Mappings and Flexible Configuration}

% - Links to HCI literature on mapping flexibility and co-creative AI systems

A main research finding in this research was that expanding the mapping capability of the generative AI system in terms of inputs, outputs, and direct mapping through the system paid dividends in terms of different instrument designs. Connecting the generative AI outputs to simultaneously control timbral parameters led to new musical potential in the intelligent MicroFreak, adapting the to a DAW exposed a convenient way to play with how AI inputs and outputs could affect live performance, and supporting simultaneous IO from multiple devices in intelligent setups expanded the gestural affordances for managing and tracking AI behaviours.

A learning of the creative practice in this research has been that mapping allowed me to find where the generative AI model could have the best musical impact within an intelligent instrument. The earlier prototypes demonstrated that the AI actions were more usefully focussed on what a human could not do (e.g., turn five knobs at once) rather than focussing on note production. Mapping the AI outputs to a visual indicator turned out to expand support better real-time understanding.

\paragraph{Design Implication:} Fast mapping enables discovery of productive AI interactions. Control should be focussed on what a musician cannot do and visual feedback can be critical.
...
The trained model itself became a design component similar to the many interchangeable devices, modules and interfaces that electronic musicians use to assemble new interfaces. The artistic process revealed new uses for the same model and the mapping functionality extended it's utility into applications quite disconnected from the originally collected data. 
Through the process of performance and practice I got to know this model better and found better ways to apply it.
This speaks to sustainability, a useful generative model can, like a trusted effects pedal or a filter module, become sustainably integrated into creative practice.
...
This research demonstrates that a new kinds of co-creative experiences are still possible, musically productive, and fun.
...
Focussing on MIDI as the communication standard took advantage of MIDI's ubiquity in music hardware to accelerate prototyping opportunities. This work focussed on connecting a generative AI model to lots of different synthesisers and systems to find out what works quickly.
It was easier to prototype an intelligent system by connecting existing hardware to an existing model in a new way, putting the focus on whether the interaction is musically satisfying.
...
\paragraph{Design implication:} Using cheap single board computers and standard MIDI lowers barriers to prototyping. 

% - Self-playing Volca through to Pi Zero deployments
% - Links to embedded instrument literature (Bela, previous Pi work) and democratization themes
% - Design implication: Standard MIDI + cheap SBCs lower barriers to intelligent instrument prototyping
...
Experience using our system with hardware synths and software DAWs in two years of live performances and recordings have demonstrated that it is feasible, flexible and musically useful.
