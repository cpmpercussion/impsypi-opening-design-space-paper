...
\begin{figure}
    \centering
    \includegraphics[width=0.5\linewidth]{figures/garage-concert-arturia.jpg}
    \caption{The stage setup for an intelligent musical instrument using our generative AI system running on a Raspberry Pi 4. The complete setup of synthesiser, Raspberry Pi, and effects pedal is battery powered. Our software runs on the Raspberry Pi 4 which interacts with the synthesiser reacting to the musicians' actions through knobs and keys on the synthesiser front panel and controlling notes and timbral parameters.}
...
Similarly, the AI system can respond to the musician's actions on standard interfaces such as piano-style keyboards as well as other controls such as knobs, sliders, levers, or movement sensors that are typically part of  electronic music devices.
...
In this work, we describe our generative AI music system showing that it is feasible, affordable and achievable for non-experts in AI to use.
We document several example intelligent musical instruments and reflections on our continuing practice with this system.
The results constitute an expansion of the design space for intelligent musical instruments, reveal pain points in setting up a generative AI instrument and introduces new possibilities for co-creation that might encourage more widespread prototyping, hacking, and musicking with AI in musical performance.
...
A small data~\cite{Vigliensoni:2022} approach to deep learning may only be part of the solution; it may be that engaging artists with systems that can easily integrate into existing practices and instruments will lead to more opportunities for data collection, training, and long-term practice.

Tahiroğlu's AI-terity instrument~\cite{tahiroglu-aiterity-2021} is a custom physical interface for exploring AI sound generation while also acting autonomously to engage the performer. The experience of performing with AI-terity was later analysed to understand how the system prompted ``unfamiliar musical expectations''~\cite{tahirouglu-evolving-2022}.
Magenta Studio~\cite{magenta-studio-2019} seeks to embed generative AI algorithms within the Ableton Live digital audio workstation (DAW) software as a plugin that can generate and manipulate MIDI recordings, although the focus is music production not perofrmance.
More broadly, Parkinson and Dunning~\cite{taxonomy-music-interactions-parkinson-2025} articulate the roles that musical machines, including intelligent musical instruments can have. They argue that such instruments can ``evolve'', ``collaborate'', and ``learn'' (among other roles). Generative AI may be well placed to help enhance these roles within new musical instruments. 

% \subsection{Embedded Musical Instruments}

Incorporating computer music systems into embedded hardware systems can allow better integration into artistic practices with platforms emerging to support this with Raspberry Pi~\cite{Berdahl:2013aa} and Beaglebone~\cite{mcpherson-bela-2016} single-board computers. Research applying generative AI on such single board computers promises to allow these technologies to be embedded within self-contained musical instruments. 
...
\begin{figure}
    \centering
    \includegraphics[width=0.5\linewidth]{figures/impsy-diagram.png}
...
This section describes musical performances where the author used this generative AI system to create intelligent musical instruments and performed with them in live concerts.
We have applied an autobiographical artistic research approach where these instruments have been developed for our own use and tested in authentic live performance situations to expand knowledge about the design space for intelligent musical isntruments.
...
The AI model was trained on a corpus an hour of musical human interaction with a single continuous controller. 
% This corresponded to around 75000 interaction events (value and time delta).
...
The AI mode, trained on expressive interaction with a continuous controller, tended to perform glassandi of various speeds, starting points and lengths on the Volca.
These sounded somewhat unusual as the synthesiser retriggered its envelope for each note.
...
\begin{figure}
	\centering
	\includegraphics[width=0.49\linewidth]{figures/intelligent-microfreak.jpg}
    ...
    Our generative AI software runs on a Raspberry Pi Zero 2 W (left) or Raspberry Pi 4 (right) and in both cases a Zoom effects pedal provides additional audio effects.
    For both setups, the generative AI system generates note data (tracking the keyboard) and seven timbral parameters (tracking knobs on the synthesiser interface).}
	\label{fig:intelligent-synth}
\end{figure}

The above example demonstrated one-way input from our system into a hardware synthesiser, other synths with MIDI outputs as well as inputs promised to support two-way interaction.
...
In many cases, simultaneous control of the synthesiser from the hardware interface and from MIDI signals is possible, so shared interaction between a performer and our AI system can be explored. 
In our case, our system was configured to behave in a call-and-response manner so that when the performer adjusted the synth controls and played notes, the AI system tracks these signals. Only when the human stops for a certain amount of time does the AI system take over the controls.

This arrangement was first trialled with an Arturia MicroFreak synthesiser with the generative AI system controlling eight parameters: note-on messages, and seven timbral parameters available to the performer via knobs on the synthesiser face.
This system was used in individual and group improvisations with performers on a variety of other instruments.
The system was highly portable (see Figure \ref{fig:intelligent-synth}) with the tiny Raspberry Pi Zero and an effect pedal accompanying the small synthesiser.
This AI model was trained on data sourced from the author's improvisation on eight continuous controllers.
In performance, the AI model produced musically satisfying material in adjusting timbral parameters and continuing notes.
Unlike a human, the AI system is able to adjust many parameters simultaneously, resulting in an inhuman but effective exploration of the synth parameter space.
In non-AI performances, the author would primarily play notes on the synthesiser with occasional timbral changes; however, the AI interactions update the timbral parameters in between phrases and in some cases in between notes, leading to a dynamic and varying performance. 
To enhance this effect, the call-and-response switch-over time was set to 0.1 seconds.

This performance concept was further explored with a different synthesiser, the Roland S-1, notable for its very small size (smaller than A5-size paper).
The size of this synth compromises the playability of the keyboard and knobs, but with the AI system, I could be less reliant on my own input on the keys, trusting that the AI system would continue sounds from where I had left off.
The timbral parameters for this synth were different than the MicroFreak with more risk of a silent performance state.
This risk led to some effort in performance to keep synth parameters in a reasonable state while allowing the AI model to explore surprising combinations of parameters.
...
\begin{figure}
    ...
    \caption{Software synthesisers used in an intelligent instrument setup. Digital audio workstation (DAW) software runs on the iPad which is connected via MIDI to our generative AI system running on a Raspberry Pi 4. The performer can interact with the instrument using the iPad touchscreen or a hardware controller interface.}
    \label{fig:intelligent-daw}
\end{figure}

While the above examples demonstrate performing electronic music with a physical hardware synthesisers, much contemporary electronic music is performed on a laptop with software synthesisers and audio files loaded into  digital audio workstation (DAW) software~\cite{daws-reuter-2021, Clauhs:2023aa}.
Similarly to synthesisers, DAW software supports MIDI input and output to control parameters, however there is generally many more routing and mapping possibilities within the DAW software itself. 
Connecting our generative AI system to computer via an inexpensive USB MIDI interface allows note data or parameters to be assigned to a wide variety of actions within different software synthesisers or to control effects or playback parameters in the DAW itself.
This represents a convenient way to design new intelligent musical instruments and interactions; both the DAW and our generative AI system are highly configurable and applicable in a wide variety of musical practices.

An example DAW and generative AI instrument is illustrated in Figure \ref{fig:intelligent-daw}. A DAW and software plugin host, Kymatica AUM (Audio Mixer), runs on an iPad with eight different sound generators available in channel strips with associated effects.
The sound generators are a mix of software synthesisers and audio file players.
The generative AI systems' MIDI interface is connected to an audio/MIDI interface connected to the iPad.
Our generative AI system is configured to respond to eight different MIDI  control change channels and generate four different channels of MIDI notes and four channels of control change data.
...
The performer can influence the AI generated data on the iPad touchscreen via a software MIDI controller or with a hardware MIDI controller connected to AUM with Bluetooth.
...
The ability to map generative AI signals over a simple MIDI connection is a  significant advantage for performing with an iPad-based setup where running arbitrary Python software is not possible. The same Raspberry Pi could also be connected to any other DAW running on a laptop computer, contrasting with other generative AI DAW approaches that involve plugins for specific DAWs~\cite{magenta-studio-2019}. 
This specific example setup has been used in solo and ensemble electronic music improvisations.
...
Signals could be mapped to areas where they were musically useful and software synths could be swapped out with others or adjusted to sound significantly different. 
...
While all parameters could be adjusted within the touchscreen interface, the addition of the hardware MIDI controller with physical knobs for influencing the generative AI system was found to be an effective way to keep track of the autonomous components of the instrument.
